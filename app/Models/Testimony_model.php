...
class Testimony_model extends Model
{
  ...
  public function __construct()
   ...
   function itemsListing(){
     $db = \Config\Database::connect("default");
     $builder = $db->table('tbl_testimonies');
       $builder->select('tbl_testimonies.*,tbl_branches.id as branch_id,tbl_branches.name as branchname');
       $builder->join('tbl_branches','tbl_branches.id=tbl_testimonies.branch');
       if($this->role!=0){
         $builder->where('branch', $this->branch);
       }
   ...
   function addNewItem($info)
   {
     ...
     $builder->insert($info);
     $this->status = 'ok';
     $this->message = 'New Testimony added successfully';
   }
