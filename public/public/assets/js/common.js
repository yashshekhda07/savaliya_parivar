...
function delete_item(e){
  ...
  switch(type){
    case 'book':
       var msg = 'You want to delete this inbox book?';
       url = baseURL+'/deleteBook/'+id;
    break;
    case 'inbox':
       var msg = 'You want to delete this inbox message?';
    ...
    break;
    case 'lists':
       var msg = 'You want to delete this list?';
       url = baseURL+'/deleteList/'+id;
    break;
    case 'groups':
       var msg = 'You want to delete this group?';
       url = baseURL+'/deleteGroup/'+id;
    break;
    case 'groupevents':
       var msg = 'You want to delete this group event?';
       url = baseURL+'/deleteGroupEvent/'+id;
    break;
    case 'listmember':
       var listid = e.target.getAttribute('data-list');
       var msg = 'You want to remove this member from this list?';
       url = baseURL+'/removeFromList/'+id+"/"+listid;
    break;
    case 'groupmember':
       var listid = e.target.getAttribute('data-list');
       var msg = 'You want to remove this member from this group?';
       url = baseURL+'/removeFromGroup/'+id+"/"+listid;
    break;
    case 'articles':
       var msg = 'You want to delete this article';
       url = baseURL+'/deleteArticle/'+id;
    break;
    case 'members':
       var msg = 'You want to delete this member details';
       url = baseURL+'/deleteMember/'+id;
    break;
    case 'coupon':
    ...
    break;
    case 'photo':
       var msg = 'You want to delete this photo';
       url = baseURL+'/deletePhoto/'+id;
    break;
    case 'testimony':
       var msg = 'You want to delete this testimony';
       url = baseURL+'/deleteTestimony/'+id;
    break;
    case 'prayer':
       var msg = 'You want to delete this prayer request';
       url = baseURL+'/deletePrayer/'+id;
    break;
...
function uploadNewAudio(event){
  ...
  var branch = $('#branch').selectpicker('val');
//  var subcategory = $('#subcategories').selectpicker('val');
  ...
  if(branch == ""){
    error_alert('Please select location/branch');
    return;
  }
...
//
$("#smsgateway").click(function () {
    if ($(this).is(":checked")) {
        $("#smsgatewaydiv").show();
         $('#smsgatewayselect').attr('required', true);
    } else {
        $("#smsgatewaydiv").hide();
         $('#smsgatewayselect').attr('required', false);
    }
});

$("#branchpicker").change(function() {
  var selected = $('#branchpicker option:selected').val();
  console.log(selected);
  load_branch_lists(selected);

  var listpicker = document.getElementById('listpicker');
  listpicker.innerHTML = '<option value="0">All Members</option>';
  $('#listpicker').selectpicker('refresh');
});

function load_branch_lists(id) {
  makeAjaxCall(baseURL + "/fetchlists/" + id, "GET", null).then(function(data) {
    var rows = data.lists;
    var listpicker = document.getElementById('listpicker');
    listpicker.innerHTML = '<option value="0">All Members</option>';
    for (var i = 0; i < rows.length; i++) {
      listpicker.innerHTML += '<option value="' + rows[i]['id'] + '">' + rows[i]['title'] + '</option>';
    }
    $('#listpicker').selectpicker('refresh');
  }, function(status) {
    console.log("failed with status", status);
